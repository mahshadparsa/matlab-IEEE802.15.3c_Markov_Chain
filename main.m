...
for i=1:size(sys_par.lambda,2)
   for j=1:size(sys_par.N,2)
    ...
    % else
         r.throughput_RCAP(i,j)    =80*1000*(1-r.b_idle(i,j) )*r.Psuccess(i,j)/r.Tservice(i,j)*sys_par.N(j);
    ...
    r.throughput_CTA(i,j)=80*1000*(1-r.b_idle_CTA(i,j))/r.Tservice_CTA(i,j)* r.N_CTA(i,j);
...
figure(4);
subplot(1,3,3),mesh(sys_par.N,sys_par.lambda,r.Psuccess);
hold on;
ylabel('\lambda');
xlabel('N');
